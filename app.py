...
#         """)
"""
Enterprise Records Management AI Dashboard (FIXED + BI PRO)
"""
...
st.title("📊 Enterprise Intelligence System")
st.caption("Power-Level AI Analytics Dashboard")
st.divider()

# ─────────────────────────────────────────────
# LOAD DATA
# ─────────────────────────────────────────────
uploaded_files = ["data/doc.pdf", "data/doc1.pdf"]
...
if run:
    ...
    st.success("BI Analysis Complete")
...
if "df" in st.session_state and "stats" in st.session_state:
    ...
    with k1:
        ...
        <div class="kpi-card">
            ...
            <div class="kpi-value">{stats["total_departments"]}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with k2:
        st.markdown(f"""
        <div class="kpi-card">
            <div class="kpi-title">🛠️ Tools</div>
            <div class="kpi-value">{stats["total_tools"]}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with k3:
        ...
        <div class="kpi-card">
            ...
            <div class="kpi-value">{stats["total_relationships"]}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col1:
        dept = st.multiselect("Departments", df["department"].unique())

    with col2:
        rel = st.multiselect("Relationships", df["relationship"].unique())
    ...
    if rel:
        filtered = filtered[filtered["relationship"].isin(rel)]
    ...
    # ─────────────────────────────────────────
    # HEATMAP (FIXED INDENT)
    # ─────────────────────────────────────────
    st.subheader("🔥 Heatmap Intelligence")

    heat = df.groupby("department").size().reset_index(name="activity")

    heat["activity"] = pd.to_numeric(heat["activity"], errors="coerce").fillna(0)
    median_val = heat["activity"].median()
    ...
    st.dataframe(
        heat.style.map(highlight, subset=["activity"]),
        use_container_width=True
    )
    ...
    draw_better_graph(df.head(1000), ax=ax2)
    st.pyplot(fig2)

else:
    st.info("📄 Run Analysis first to load dashboard")

  # ─────────────────────────────────────────
# 🏢 Department Detail Cards (FULL FIXED VERSION)
# ─────────────────────────────────────────
...
if "df" in st.session_state:
    ...
    for i, dept in enumerate(depts):
        ...
        with cols[i % 3]:
            ...
            <div class="department-card">
                ...
                <div class="card-sub">
                    {icons.get(rel,'⚪')} <b>{rel}</b>
                </div>
                <div class="card-sub">
                    <b>Tools:</b> {' · '.join(tools) if tools else "N/A"}
                </div>
            </div>
            """, unsafe_allow_html=True)
    ...
    st.info("📄 Run AI Analysis first to generate department cards")
...
if df is not None and not df.empty:
    ...
    st.download_button(
        ...
        data=buffer,
        file_name="enterprise_ai_report.xlsx"
    )
...
#    # ─────────────────────────────────────────
# # 🏢 Department Detail Cards (SAFE VERSION)
# # ─────────────────────────────────────────
...
# else:
#     st.info("📄 Run AI Analysis first to generate data")
...
#         # MAIN SHEET (must exist)
#         df.to_excel(writer, index=False, sheet_name="Full Data")

#         # OPTIONAL SHEETS (safe)
#         if "relationship" in df.columns:
#             df.groupby("relationship").size().reset_index(name="count").to_excel(
#                 writer, sheet_name="Relationships", index=False
#             )

#         if "department" in df.columns:
#             df.groupby("department").size().reset_index(name="count").to_excel(
...
# else:
#     st.warning("⚠️ No data available for export. Run analysis first.")